...
def process_allfile(file_path):
    ...
    for seed in range(seeds):
        ...
        test_outputs_norm_mea=dRVFL_predict(best_hypers,datastr,train_idx,test_idx,1,seed)
    ...
    rvflpre=np.concatenate(all_predictions,axis=1)
    rvflpre=pd.DataFrame(rvflpre)

    
    
    
    output_file = os.path.join(result_path, os.path.basename(file_path))
    rvflpre.to_csv(output_file)
