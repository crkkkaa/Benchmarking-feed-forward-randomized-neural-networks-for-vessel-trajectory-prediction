...
def edRVFL_predict(hyper,data,train_idx,test_idx,s):
    ...
    last_states=None
    outputs=np.zeros((len(test_idx),Nl,2))
    ...
    for l in range(Nl):
        ...
        outputs[:, l, :] = test_outputs_norm
        
    return np.median(outputs,axis=1)
...
def process_allfile(file_path):
    ...
    for seed in range(seeds):
        ...
        test_outputs_norm_mea=edRVFL_predict(best_hypers,datastr,train_idx,test_idx,seed)

        predictions=yscaler.inverse_transform(test_outputs_norm_mea)
        if dif1st:
            predictions=predictions.ravel()+target[-testl-1:-1]
        all_predictions.append(predictions)
    edrvflpre=np.concatenate(all_predictions,axis=1)
    edrvflpre=pd.DataFrame(edrvflpre)
    output_file = os.path.join(result_path, os.path.basename(file_path))
    edrvflpre.to_csv(output_file)
